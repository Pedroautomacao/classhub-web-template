...
import {
  ...
  IconButton, MenuItem, Select, Stack, TextField, Tooltip,
  Typography, InputLabel, FormControl, Divider, Alert,
} from '@mui/material'
import { Edit, Cancel, Add, Delete } from '@mui/icons-material'
        ...
        <Stack spacing={2}>
          ...
          <Grid container spacing={2}>
            <Grid size={{ xs: 12, sm: 4 }}>
              <TextField
                label="Período – início *" type="date" fullWidth size="small"
            ...
            <Grid size={{ xs: 12, sm: 4 }}>
              <TextField
                label="Período – fim *" type="date" fullWidth size="small"
                slotProps={{ inputLabel: { shrink: true } }}
          ...
          {/* Add session */}
          <Typography variant="subtitle2" fontWeight={600}>Adicionar Aulas</Typography>
              ...
              {sessions.map((s, i) => (
                <Stack key={i} direction="row" alignItems="center" justifyContent="space-between"
                  ...
                  <Typography variant="body2">
                    <strong>{dayjs(s.lesson_date).format('DD/MM/YYYY')}</strong>
                    {' · '}{s.class_name}
          ...
          {sessions.length === 0 && (
            <Alert severity="info" sx={{ py: 0.5 }}>
              Adicione pelo menos uma aula para criar o fechamento.
            </Alert>
          )}
      ...
      <DialogContent>
        <Stack component="form" id="edit-closing-form" onSubmit={handleSubmit((v) => mutation.mutate(v))} spacing={2} sx={{ pt: 1 }}>
          <Typography variant="body2" color="text.secondary">
            Período: {closing.date_from} – {closing.date_to}
  ...
  return (
    <Box>
      ...
      <ConfirmDialog
        ...
        message={`Cancelar a solicitação do período ${cancelTarget?.date_from} – ${cancelTarget?.date_to}?`}
